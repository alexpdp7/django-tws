...
def run(module):
    ...
    parser.add_argument("--no-reset-database", action="store_false", dest="reset_database")
    parser.add_argument("--keep-database", action="store_true", dest="keep_database")
    args = parser.parse_args()

    with with_reset_database(module, db, env, reset_database=args.reset_database, keep_database=args.keep_database):
       subprocess.run([sys.executable, "-m", f"{module}.manage", args.command], check=True, env=env)
